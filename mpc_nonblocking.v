...
Inductive exp : Type :=
 ...
 | seq_exp : exp -> exp -> exp
 | val_exp : id -> exp
 | nreceive_exp : id -> ty -> exp -> exp -> exp
 | future_exp : ty -> exp.
...
Inductive value : exp -> Prop :=
 ...
 | process_val : forall x,
   value (val_exp x)
 | future_val : forall T, value (future_exp T).
...
match e with
 ...
 | val_exp p => val_exp p
 | stop_exp => stop_exp
 | (nreceive_exp m T' e1 e2) => (nreceive_exp m T' 
   (if eq_id_dec x m then e1 else (subst x e' e1)) 
   (subst x e' e2))
 | future_exp T' => future_exp T'
end.
...
Inductive has_type : typing_env -> exp -> ty -> cty -> Prop :=
 ...
 | T_nreceive_exp : forall x e e' T T' C' C'' Gamma,
   has_type (extend Gamma x T) e T' C' ->
   In C' [r_cty T ; C'] ->
   has_type Gamma e' T' C'' ->
   has_type Gamma (nreceive_exp x T e e') T' (choice (seq [r_cty T ; C']) C'').
...
Inductive future_value : exp -> Prop :=
| f_v : forall T, value (future_exp T) ->
          future_value (future_exp T).
...
Inductive step : global_conf -> action -> global_conf -> Prop :=
 ...
 | ST_seq_exp2 : forall T x st' e1 M M' p P P' a e2 
   ...
   (Sigma st'' e2 M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P')
| ST_nreceive_exp1 : forall T T' x' x e e' p st'
   e'' M' v' P v'',
   value (future_exp T) ->
   future_value (future_exp T) ->
   step (Sigma (st_decl T' x' v'') 
   (nreceive_exp x T e e') (nil) (val_exp p) |: 
   (Sigma st' e'' M' v') |: P) 
   (receive (future_exp T) (val_exp p))
   (Sigma (st_decl T' x' v'') (subst x (future_exp T) e) nil 
   (val_exp p) |: (Sigma st' e'' M' v') |: P)
 | ST_nreceive_exp2 : forall v T T' x' x e e' M p st'
   e'' M' v' P v'',
   step (Sigma (st_decl T' x' v'') 
   (nreceive_exp x T e e') (v :: M) (val_exp p) |: 
   (Sigma st' e'' M' v') |: P) 
   (receive v (val_exp p))
   (Sigma (st_decl T' x' v'') (receive_exp x T e e') (v :: M) 
   (val_exp p) |: (Sigma st' e'' M' v') |: P)
 | ST_nreceive_exp3 : forall v T T' x' e e' x M p 
   st' e'' M' v' v'' P,
   step (Sigma (st_decl T' x' v'') 
   (nreceive_exp x T e e') nil (val_exp p) |: 
   (Sigma st' e'' M' v') |: P) 
   ...
   (Sigma (st_decl T' x' v'') (e') (v :: M) 
   (val_exp p) |: (Sigma st' e'' M' v') |: P)
 | F_send_exp : forall v v'' v''' st' e' M' x T
   M p T' P,
   value v ->
   future_value (future_exp T) ->
   step (Sigma (st_decl T' x v''') (send_exp v (future_exp T)) 
   (v'' :: M) (val_exp p) |: 
   (Sigma st' e' M' v) |: P) 
   (send (future_exp T) (val_exp p) v)
   (Sigma (st_decl T' x v''') 
   (receive_exp x T (send_exp v (var_exp x)) (send_exp v (future_exp T))) 
   (v'' :: M) (val_exp p) |: P)
 | F_set_exp : forall v v'' v''' st' e' M' x T T'
   M p P,
   value v ->
   future_value (future_exp T) ->
   step (Sigma (st_decl T' x v''') (set_exp (future_exp T)) 
   (v'' :: M) (val_exp p) |: 
   (Sigma st' e' M' v) |: P) 
   (set (val_exp p))
   (Sigma (st_decl T' x v''') 
   (receive_exp x T (set_exp (var_exp x)) (set_exp (future_exp T)))
   (v'' :: M) (val_exp p) |: P)
 | F_fst_exp : forall v v'' v''' st' e' M' x T
   M p P T',
   value v ->
   future_value (future_exp T) ->
   step (Sigma (st_decl T' x v''') (fst_exp (pair_exp (future_exp T) v)) 
   (v'' :: M) (val_exp p) |: 
   (Sigma st' e' M' v) |: P) 
   (local (val_exp p))
   (Sigma (st_decl T' x v''') 
   (receive_exp x T (fst_exp (pair_exp (var_exp x) v)) 
   (fst_exp (pair_exp (future_exp T) v))) 
   (v'' :: M) (val_exp p) |: P)
 | F_snd_exp : forall v v'' v''' st' e' M' x T
   M p P T',
   value v ->
   future_value (future_exp T) ->
   step (Sigma (st_decl T' x v''') (snd_exp (pair_exp v (future_exp T))) 
   (v'' :: M) (val_exp p) |: 
   (Sigma st' e' M' v) |: P) 
   (local (val_exp p))
   (Sigma (st_decl T' x v''') 
   (receive_exp x T (snd_exp (pair_exp v (var_exp x))) 
   (snd_exp (pair_exp v (future_exp T)))) 
   (v'' :: M) (val_exp p) |: P)
 | F_abs_exp : forall v v'' v''' e st' e' M' x T
   M p P T',
   value v ->
   future_value (future_exp T) ->
   step (Sigma (st_decl T' x v''') (app_exp (abs_exp x T e) (future_exp T)) 
   (v'' :: M) (val_exp p) |: 
   (Sigma st' e' M' v) |: P) 
   (local (val_exp p))
   (Sigma (st_decl T' x v''') 
   (receive_exp x T (app_exp (abs_exp x T e) (var_exp x)) 
   (app_exp (abs_exp x T e) (future_exp T))) 
   (v'' :: M) (val_exp p) |: P)
 | F_seq_exp : forall v v'' v''' e st' e' M' x T
   M p P T',
   value v ->
   future_value (future_exp T) ->
   step (Sigma (st_decl T' x v''') (seq_exp (future_exp T) e) 
   (v'' :: M) (val_exp p) |: 
   (Sigma st' e' M' v) |: P) 
   (local (val_exp p))
   (Sigma (st_decl T' x v''') e (v'' :: M) 
...
Inductive appears_free_in : id -> exp -> Prop :=
 ...
 | afi_nreceive_exp1 : forall T e e' x x',
   x <> x' ->
   appears_free_in x' e ->
   appears_free_in x' (nreceive_exp x T e e')
 | afi_nreceive_exp2 : forall T e e' x x',
   appears_free_in x' e' ->
   appears_free_in x' (nreceive_exp x T e e').
...
- right. 
 destruct (v :: M). 
 exists (st_decl T' x' v''').
 exists (subst x (future_exp T) e). exists nil.
 exists st'. exists e'.
 exists M'. exists v'.
 exists (receive (future_exp T) (val_exp p)). exists P.
 exists T. intros. 
 apply ST_nreceive_exp1. 
 apply future_val. apply f_v.
 apply future_val. exists (st_decl T' x' v''').
 exists (receive_exp x T e e'0). exists (e0::l).
 exists st'. exists e'.
 exists M'. exists v'.
 exists (receive e0 (val_exp p)). exists P.
 exists T. intros.
 apply ST_nreceive_exp2.
Qed.
...
- inversion HE; subst.
 ...
 assumption. apply seq_subtype_cty2.
 assumption. inversion HT. inversion H4.
(* Snd *) 
- inversion HE; subst.
 apply T_snd_exp with T.
 apply IHHT in H12. assumption.
 ...
 apply subsume_cty with C'.
 assumption. apply seq_subtype_cty2.
 assumption. inversion HT. inversion H5.
(* App *)
- inversion HE; subst.  
...
- inversion HE; subst. apply IHHT1 in H15. 
 ...
 inversion HT1. 
(* nReceive *)
- inversion HE; subst. apply T_receive_exp.
assumption. assumption. assumption.
Qed.
