...
match e with
 ...
 | val_exp p => val_exp p
 | stop_exp => stop_exp
 | wait_exp e => (wait_exp (subst x e' e))
end.
...
Inductive has_type : typing_env -> exp -> ty -> cty -> Prop :=
 ...
 | T_wait_exp : forall Gamma e T C,
    has_type Gamma e (process_ty T C) null ->
    has_type Gamma (wait_exp e) T C.
...
Inductive step : global_conf -> action -> global_conf -> Prop :=
 ...
 | ST_seq_exp2 : forall T x st' e1 M M' p P P' a e2 
   ...
   (Sigma st'' e2 M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P')
 | ST_wait_exp1 : forall v v' v''' x T
   M p P v'' M' st',
   value v'' ->
   step (Sigma (st_decl T x v''') (wait_exp v)
   (v' :: M) (val_exp p) |: (Sigma st' v'' M' v) |: P) 
   (local (val_exp p)) 
   (Sigma (st_decl T x v''') v'' (v' :: M) (val_exp p) |: 
   (Sigma st' v'' M' v) |: P)
 | ST_wait_exp2 : forall v v' v''' st' e' M' x T
   M p P M'' e1 e1' st'' M''' e''' st''' P' a,
   step (Sigma (st_decl T x v''') e1 (v' :: M) 
   (val_exp p) |: (Sigma st' e' M' v) |: P) a 
   (Sigma st'' e1' M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P') ->
   step (Sigma (st_decl T x v''') (wait_exp e1)
   (v' :: M) (val_exp p) |: (Sigma st' e' M' v) |: P) 
   a 
   (Sigma st'' (wait_exp e1') M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P').
...
Inductive appears_free_in : id -> exp -> Prop :=
 ...
 | afi_wait_exp : forall x e,
   appears_free_in x e ->
   appears_free_in x (wait_exp e).
...
Axiom wait_exp_substitution : forall v v'' st e M p st' e' M' v' P,
value e ->
fresh v'' ((Sigma st (wait_exp e) (v :: M) (val_exp p)) |: 
(Sigma st' e' M' v') |: P) ->
wait_exp v' = wait_exp e.

Axiom wait_value : forall v v'' st e M p st' e' M' v' P,
value e ->
fresh v'' ((Sigma st (wait_exp e) (v :: M) (val_exp p)) |: 
(Sigma st' e' M' v') |: P) ->
value e'.
...
- (* Wait *)
 right. 
 destruct IHhas_type. inversion H1.
 apply fresh_address. 
 assumption. reflexivity.
 exists (st_decl T' x' v'''). 
 exists e'. exists (v :: M).
 ...
 exists v'. exists (local (val_exp p)).
 exists P. exists T'. 
 intros. replace (wait_exp e) with (wait_exp v').
 apply ST_wait_exp1.
 apply wait_value with v v'' st e M p st' M' v' P. assumption.
 assumption. 
 apply wait_exp_substitution with v v'' st M p st' e' M' P.
 assumption. assumption. 
 destruct H3. exists x.
 destruct H3. exists (wait_exp x0).
 destruct H3. exists x1. 
 destruct H3. exists x2. destruct H3. 
 exists x3. destruct H3. exists x4.
 destruct H3. exists x5. 
 destruct H3. exists x6. 
 destruct H3. exists x7. 
 destruct H3. exists x8. intros.
 apply ST_wait_exp2. 
 apply H3. assumption.
Qed.
...
((Sigma st'' e'' M'' (val_exp p)) |: 
(Sigma st''' e''' M''' v') |: P') ->
exists T'', exists C', has_type empty e' T'' C' ->
has_type empty e'' T C.
Proof with eauto.
 remember (@empty ty) as Gamma.
...
(* Send *)
- inversion HE; subst...
 intros. exists T'. exists C'. intros.
 apply subsume_cty with C'.
 assumption. apply seq_subtype_cty2.
 assumption.  apply IHHT2 in H24. 
 destruct H24. destruct H2. exists x.
 exists x0. intros. 
 apply T_send_exp with T C. 
 assumption. apply H2.
 assumption. assumption. 
 assumption. assumption. reflexivity.
 apply IHHT1 in H24. destruct H24. 
 exists x. destruct H2. exists x0.
 intros.
 apply T_send_exp with T C.
 apply H2.
 assumption. 
 assumption.
 assumption. assumption. assumption.
 reflexivity.  apply IHHT2 in H16.
 destruct H16. exists x. 
...
- inversion HE; subst. 
 ...
 apply reflexive_subtype_cty.
(* Become *)
- inversion HE; subst. exists T. exists C. intros.
 assumption.
(* Set *)
- inversion HE; subst. exists T'. exists C'. intros.
 apply subsume_ty with T'0. 
 assumption. assumption. 
 apply IHHT in H14.
 destruct H14. exists x. 
...
- inversion HE; subst.
 ...
 apply T_val_exp. 
 apply null_subtype_cty.
(* Pair *)
- inversion HE; subst.  
 apply IHHT1 in H15. 
 destruct H15. exists x.
 destruct H1. exists x0. intros.
 apply T_pair_exp. apply H1. assumption.
 assumption. assumption.
 assumption. reflexivity.
 apply IHHT2 in H23. 
 destruct H23. exists x.
 destruct H1. exists x0. intros.
 apply T_pair_exp.  
 assumption. apply H1.
 assumption.  assumption.
...
(* Fst *)
- inversion HE; subst.
 apply IHHT in H12. destruct H12.
 exists x. destruct H. exists x0. intros.
 apply T_fst_exp with T'0.
 apply H.
 assumption.
...
- inversion HE; subst. 
 ...
 apply subsume_cty with C''.
 assumption. 
 apply seq_subtype_cty2. rewrite -> H5. 
 assumption. assumption. 
 apply IHHT1 in H16.
 destruct H16. exists x. 
 destruct H2. exists x0. intros.
 apply T_app_exp with T. apply H2. 
 assumption. assumption. assumption.
 assumption. assumption. reflexivity.
 apply IHHT2 in H24. 
 destruct H24. exists x. 
 destruct H2. exists x0.
 intros.
 apply T_app_exp with T. 
 assumption. apply H2.  
...
- inversion HE; subst.
 ...
 destruct H16. exists x.
 destruct H1. exists x0. intros.
 apply T_if_exp. apply H1. assumption.
 assumption. assumption.
 assumption. 
 assumption. reflexivity. 
(* Seq *)
- inversion HE; subst.  
 ...
 destruct H15. exists x. 
 destruct H1. exists x0. intros. 
 apply T_seq_exp with T. apply H1. 
 assumption. assumption. assumption.
 assumption. reflexivity.
 exists T. exists C. intros.
 apply subsume_cty with C'.
...
(* Wait *)
 - inversion HE; subst.
 exists T. exists C. intros.
 assumption. apply IHHT in H12.
 destruct H12. 
 exists x. destruct H. exists x0.
 intros. apply T_wait_exp. apply H.
 assumption. reflexivity.
Qed.
...
Theorem soundness: forall v0 T'' T T' T''' C' x' v''' v'' e v M p st' e' M' P tr st'' 
e'' M'' v' st''' e''' M''' P' C,
value v ->
value v0 ->
value v'' ->
...
Proof.
 intros v0 T'' T T' T''' C' x' v''' v'' e v M p st' e' M' P tr st'' 
 e'' M'' v' st''' e''' M''' P' C. 
 intros H1 H2 H3 H4 H5 Hhas_type Hmulti.
...
M'0 v v'' v'''0 v0 x'0 p0 C v' in H5 .
...
Lemma reduction_wait2 : forall T x v''' e v'' M p st' e' M' P 
a st'' e'' M'' st''' e''' M''' v' P',reduction (Sigma (st_decl T x v''') e (v'' :: M)(val_exp p) |: (Sigma st' e' M' v') |: P) a (Sigma st'' e'' M'' (val_exp p) |:(Sigma st''' e''' M''' v') |: P') ->reduction (Sigma (st_decl T x v''') (wait_exp e) (v'' :: M) (val_exp p) |:(Sigma st' e' M' v') |: P) a(Sigma st'' (wait_exp e'') M'' (val_exp p) |: (Sigma st''' e''' M''' v') |: P').
Proof.
 intros.
 inversion H. subst.
 apply one_step_red.
 ...
 assumption.
 apply refl_red. subst.
 apply trans_red with st''0 (wait_exp e''0) M''0 P'0 st'''0 e'''0 M'''0.
 apply one_step_red. apply ST_wait_exp2.
 inversion H13. subst.
 assumption. inversion H21. subst.
 apply one_step_red. apply ST_wait_exp2.
 assumption.
