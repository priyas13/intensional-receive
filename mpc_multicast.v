...
(* This lemma states that if x2 extends typing context ctxt and 
...
match e with
 ...
 | val_exp p => val_exp p
 | stop_exp => stop_exp
 | multicast_exp e e3 => (multicast_exp e (subst x e' e3))
end.
...
Inductive has_type : typing_env -> exp -> ty -> cty -> Prop :=
 ...
 | T_multicast_exp : forall Gamma e1 e2 e T T' T'' T1 T2 T3 C1 C2 Cn C C' C'' C''',
   has_type Gamma e T C' ->
   has_type Gamma e1 (process_ty T1 C1) C ->
   has_type Gamma e2 T'' C'' ->
   is_mem_cty (r_cty T) (seq (C1 :: C2 :: Cn)) true ->
   T'' = (prod_ty (process_ty T2 C2) T3) ->
   updated_cty (r_cty T') (seq (C1 :: C2 :: Cn)) C''' ->
   In C' (append_cty (C' :: (seq [C ; C'']) :: s_cty T' :: nil) (C''')) ->
   In C (append_cty (C' :: (seq [C ; C'']) :: s_cty T' :: nil) (C''')) ->
   In C'' (append_cty (C' :: (seq [C ; C'']) :: s_cty T' :: nil) (C''')) ->
   In C [C ; C''] ->
   In C'' [C ; C''] ->
   has_type Gamma (pair_exp e1 e2) (prod_ty (process_ty T1 C1) T'') (seq [C ; C'']) ->
   has_type Gamma (multicast_exp (pair_exp e1 e2) e) T 
   (seq (append_cty (C' :: (seq [C ; C'']) :: s_cty T' :: nil) (C'''))).
...
Fixpoint update_one_mail (v : exp) (e : exp) (P1 : global_conf) : global_conf :=
match P1 with 
 | empty_comp _ => empty_comp _
 | (Sigma sts e' M' p1) |: P1'  => if (is_equal_exp e p1) then (Sigma sts e' (append_mail_box M' v) p1) |: P1'
 else (Sigma sts e' M' p1) |: (update_one_mail v e P1')
end.

Fixpoint update_one_mail' (v : exp) (e : exp) (P1 : process_conf) : mail_box :=
match P1 with 
| (Sigma sts e' M' p1) => match e with 
                                         | pair_exp e'' el' => if (is_equal_exp e'' p1) then (append_mail_box M' v) 
                                           else update_one_mail' v el' (Sigma sts e' M' p1)
                                         | send_exp e'' el' => M'
                                         | spawn_exp x T' e'' => M'
                                         | become_exp e'' => M'
                                         | nat_exp n => M'
                                         | unit_exp => M'
                                         | receive_exp x T' e'' el' => M'
                                         | set_exp e'' => M'
                                         | get_exp => M'
                                         | self_exp => M'
                                         | stop_exp => M'
                                         | multicast_exp e'' el' => M'
                                         | abs_exp x T' e''=> M'
                                         | app_exp e'' el' => M'
                                         | fst_exp e'' => M'
                                         | snd_exp e'' => M'
...
Fixpoint update_some_mail (v : exp) (el : exp) (P1 : global_conf) : global_conf :=
match P1 with 
 | empty_comp _ => empty_comp _
 | (Sigma sts e' M' p1) |: P1'  => match el with 
                                                    | pair_exp e'' el' => app (update_one_mail v e'' P1) (update_some_mail v el' P1)
                                                    | send_exp e'' el' => (Sigma sts e' M' p1) |: P1'
                                                    | spawn_exp x T' e'' => (Sigma sts e' M' p1) |: P1'
                                                    | become_exp e'' => (Sigma sts e' M' p1) |: P1'
                                                    | nat_exp n => (Sigma sts e' M' p1) |: P1'
                                                    | unit_exp => (Sigma sts e' M' p1) |: P1'
                                                    | receive_exp x T' e'' el' => (Sigma sts e' M' p1) |: P1'
                                                    | set_exp e'' => (Sigma sts e' M' p1) |: P1'
                                                    | get_exp => (Sigma sts e' M' p1) |: P1'
                                                    | self_exp => (Sigma sts e' M' p1) |: P1'
                                                    | stop_exp => (Sigma sts e' M' p1) |: P1'
                                                    | multicast_exp e'' el' => (Sigma sts e' M' p1) |: P1'
                                                    | abs_exp x T' e''=> (Sigma sts e' M' p1) |: P1'
                                                    | app_exp e'' el' => (Sigma sts e' M' p1) |: P1'
                                                    | fst_exp e'' => (Sigma sts e' M' p1) |: P1'
                                                    | snd_exp e'' => (Sigma sts e' M' p1) |: P1'
                                                    | true_exp => (Sigma sts e' M' p1) |: P1'
                                                    | false_exp => (Sigma sts e' M' p1) |: P1'
                                                    | var_exp e => (Sigma sts e' M' p1) |: P1'
                                                    | if_exp e'' e''' el' => (Sigma sts e' M' p1) |: P1'
                                                    | seq_exp e'' el' => (Sigma sts e' M' p1) |: P1'
                                                    | val_exp x => (Sigma sts e' M' p1) |: P1'
end
end.
...
Inductive step : global_conf -> action -> global_conf -> Prop :=
 ...
 | ST_seq_exp2 : forall T x st' e1 M M' p P P' a e2 
   ...
   (Sigma st'' e2 M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P')
 | ST_multicast_exp1 : forall v v' v'' v''' st' e' M' x T
   M p P v1 v2,
   value v1 ->
   value v2 ->
   value v'' ->
   step (Sigma (st_decl T x v''') (multicast_exp (pair_exp v1 v2) v'') 
   (v :: M) (val_exp p) |: 
   (Sigma st' e' M' v') |: P)
   (multicast v'' (val_exp p) (pair_exp v1 v2))
   (Sigma (st_decl T x v''') v'' (v :: M) (val_exp p) |: 
   (Sigma st' e' (update_one_mail' v'' (pair_exp v1 v2) (Sigma st' e' M' v')) v') |:
   update_some_mail v'' (pair_exp v1 v2) P)
 | ST_multicast_exp2 : forall v v' v'' v''' st' e' M' x T
   M p P M'' e1 e2 e1' st'' M''' e''' st''' P' a,
   value v'' ->
   step (Sigma (st_decl T x v''') e1 (v' :: M) 
   (val_exp p) |: (Sigma st' e' M' v) |: P) a 
   (Sigma st'' e1' M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P') ->
   step (Sigma (st_decl T x v''') (multicast_exp (pair_exp e1 e2) v'')
   (v' :: M) (val_exp p) |: (Sigma st' e' M' v) |: P) 
   a 
   (Sigma st'' (multicast_exp (pair_exp e1' e2) v'') M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P')
| ST_multicast_exp3 : forall v v' v'' v''' st' e' M' x T
   M p P M'' e1 e2 e2' st'' M''' e''' st''' P' a,
   value v'' ->
   step (Sigma (st_decl T x v''') e2 (v' :: M) 
   (val_exp p) |: (Sigma st' e' M' v) |: P) a 
   (Sigma st'' e2' M'' (val_exp p) |: 
   ...
   (Sigma st'' (multicast_exp (pair_exp e1 e2') v'') M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P')
 | ST_multicast_exp4 : forall v v' v''' st' e' M' x T
   M p P M'' e1 e2 e3 e3' st'' M''' e''' st''' P' a,
   step (Sigma (st_decl T x v''') e3 (v' :: M) 
   (val_exp p) |: (Sigma st' e' M' v) |: P) a 
   (Sigma st'' e3' M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P') ->
   step (Sigma (st_decl T x v''') (multicast_exp (pair_exp e1 e2) e3)
   (v' :: M) (val_exp p) |: (Sigma st' e' M' v) |: P) 
   a 
   (Sigma st'' (multicast_exp (pair_exp e1 e2) e3') M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P').
...
Inductive appears_free_in : id -> exp -> Prop :=
 ...
 | afi_multicast_exp1 : forall e1 e2 x,
   appears_free_in x e1 ->
   appears_free_in x (multicast_exp e1 e2)
 | afi_multicast_exp2 : forall e1 e2 x,
   appears_free_in x e2 ->
   appears_free_in x (multicast_exp e1 e2).
...
apply T_val_exp.
- (* Multicast *)
 apply T_multicast_exp with T'' T1 T2 T3 C1 C2 Cn . 
 apply IHhas_type1.
 intros. rewrite <- Heqv. reflexivity.
 apply afi_multicast_exp2. assumption.
 apply IHhas_type2. intros. rewrite <- Heqv.
 reflexivity. apply afi_multicast_exp1.
 apply afi_pair_exp1. assumption.
 apply IHhas_type3. intros. rewrite <- Heqv.
 reflexivity. apply afi_multicast_exp1.
 apply afi_pair_exp2. assumption. assumption.
 assumption. assumption. assumption. assumption.
 assumption. assumption. assumption.
 apply IHhas_type4. intros. 
 rewrite <- Heqv. reflexivity.
 apply afi_multicast_exp1. assumption.
Qed.
...
- (* Multicast *)
 right. 
 destruct IHhas_type1.
 apply fresh_address. inversion H1.
 assumption. reflexivity. 
 destruct IHhas_type2.
 apply fresh_address. inversion H1.
 assumption. reflexivity. 
 destruct IHhas_type3.
 apply fresh_address. inversion H1.
 assumption. reflexivity.
 exists (st_decl T' x' v''').
 exists e. exists (v :: M). 
 exists st'. exists e'.
 exists (update_one_mail' e (pair_exp e1 e2) (Sigma st' e' M' v')). 
 exists v'. 
 exists  (multicast e (val_exp p) (pair_exp e1 e2)).
 exists (update_some_mail e (pair_exp e1 e2) P).
 exists T. intros.
 apply ST_multicast_exp1. assumption. 
 assumption. assumption.
 inversion H11. exists x. inversion H12. 
 exists (multicast_exp (pair_exp e1 x0) e).
 inversion H13. exists x1. inversion H14. 
 exists x2. inversion H15.
 exists x3. inversion H16. exists x4.
 inversion H17. exists x5. inversion H18.
 exists x6. inversion H19. exists x7. 
 inversion H20. exists x8. intros. 
 apply ST_multicast_exp3.
 assumption. apply H21. assumption.
 inversion H10. exists x. inversion H11.
 exists (multicast_exp (pair_exp x0 e2) e).
 inversion H12. exists x1. inversion H13. 
 exists x2. inversion H14.
 exists x3. inversion H15. exists x4. 
 inversion H16. exists x5. inversion H17.
 exists x6. inversion H18. exists x7. 
 inversion H19. exists x8. intros.
 apply ST_multicast_exp2. assumption.
 apply H20. assumption.
 inversion H3. exists x. 
 inversion H10. exists (multicast_exp (pair_exp e1 e2) x0).
 inversion H11. exists x1. 
 inversion H12. exists x2. inversion H13.
 exists x3. inversion H14. exists x4. 
 inversion H15. exists x5. inversion H16.
 exists x6. inversion H17. exists x7. 
 inversion H18. exists x8. intros.
 apply ST_multicast_exp4. 
...
- apply T_val_exp.
- (* Multicast *)
 apply T_multicast_exp with (prod_ty (process_ty T2 C2) T3)
 T1 T2 T3 C1 C2 Cn.
 apply IHe2.
 assumption. inversion H16. subst.
 apply context_invariance with (extend Gamma x T').
 assumption. intros.
 unfold extend. rename x into self_st. 
 rewrite neq_id... apply not_equal.
 apply context_invariance with (extend Gamma x T').
 assumption. intros.
 unfold extend. rename x into self_st. 
 rewrite neq_id... apply not_equal.
 ...
 apply context_invariance with (extend Gamma x T'). 
 assumption.
 intros. unfold extend. rename x into self_st.
 rewrite neq_id. reflexivity.
 apply not_equal.
Qed.
...
- inversion HE; subst.  
 ...
 apply T_multicast_exp with (prod_ty (process_ty T2 C2) T3)
 T1 T2 T3 C1 C2 Cn.
 apply IHHT2 in H29. assumption.
 reflexivity. apply IHHT2 in H29. assumption.
 reflexivity. assumption. assumption.
 reflexivity.
 assumption. assumption. assumption.
 assumption. assumption. assumption.
 ...
 T1 T2 T3 C1 C2 Cn.
 assumption. assumption.  
 apply IHHT3 in H29. assumption.
 reflexivity. assumption. reflexivity. assumption.
 assumption. assumption. assumption.
 assumption. assumption.
 apply T_pair_exp. assumption.
 apply IHHT3 in H29. assumption.
 reflexivity. assumption. assumption.
 apply T_multicast_exp with 
 (prod_ty (process_ty T2 C2) T3)
 T1 T2 T3 C1 C2 Cn.
 apply IHHT1 in H21. assumption.
...
Lemma mover_properties : forall a a' tr v p p' P P' tr1 tr2 K K1' K2' K'',
                   ...
                   ((a = (set p) -> lipton_reorder (append_actions tr1 (append_actions [a ; a'] tr2)) (append_actions tr1 (append_actions [a' ; a] tr2))))) \/
                   ((a' = (multicast v p p') -> lipton_reorder (append_actions tr1 (append_actions [a ; a'] tr2)) (append_actions tr1 (append_actions [a' ; a] tr2)))).
  Proof.
  intros.
  left.
  intros.
...
Lemma reduction_multicast1 : forall T x v''' v'' M p st' e' M' P 
v' v1 v2 v,value v1 ->
value v2 ->
value v'' ->reduction  (Sigma (st_decl T x v''') (multicast_exp (pair_exp v1 v2) v'') 
(v :: M) (val_exp p) |: (Sigma st' e' M' v') |: P)
[(multicast v'' (val_exp p) (pair_exp v1 v2))]
(Sigma (st_decl T x v''') v'' (v :: M) (val_exp p) |: 
(Sigma st' e' (update_one_mail' v'' (pair_exp v1 v2) (Sigma st' e' M' v')) v') |:
...
Proof.
 intros.
 apply one_step_red.
 apply ST_multicast_exp1.
 assumption. assumption.
 assumption.
Qed.

Lemma reduction_multicast2 : forall T x v''' M p st' e' M' P a st'' M'' 
st''' e''' M''' v' P' v e2 e1 e3' e3,reduction (Sigma (st_decl T x v''') e3 (v' :: M) 
(val_exp p) |: (Sigma st' e' M' v) |: P) a 
(Sigma st'' e3' M'' (val_exp p) |: 
(Sigma st''' e''' M''' v) |: P') ->
reduction (Sigma (st_decl T x v''') (multicast_exp (pair_exp e1 e2) e3)
(v' :: M) (val_exp p) |: (Sigma st' e' M' v) |: P) 
a 
(Sigma st'' (multicast_exp (pair_exp e1 e2) e3') M'' (val_exp p) |: 
(Sigma st''' e''' M''' v) |: P').
Proof.
 ...
 assumption. subst.
 apply refl_red. subst.
 apply trans_red with st''0 (multicast_exp (pair_exp e1 e2) e'') M''0 P'0 st'''0 e'''0 M'''0.
 apply one_step_red. apply ST_multicast_exp4.
