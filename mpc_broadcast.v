...
match e with
 ...
 | val_exp p => val_exp p
 | stop_exp => stop_exp
 | broadcast_exp e =>  broadcast_exp (subst x e' e)
end.
...
Fixpoint update_all_mail (v : exp) (P : global_conf) : global_conf :=
match P with 
 | empty_comp _ => P
 | (Sigma sts e M' p1) |: P1' => (Sigma sts e (append_mail_box M' v) p1) |:
   (update_all_mail v P1')
end.
...
match P with 
 ...
 | (Sigma sts e M' p1) |: P1' => match is_equal_exp 
   p p1 with 
 | left_ => true
 | right_ => is_mem_process_global_conf p (P1')
end
end.*)
...
Inductive has_type : typing_env -> exp -> ty -> cty -> Prop :=
 ...
 | T_broadcast_exp : forall Gamma e T C,
   has_type Gamma e T C ->
   In C (C :: [(s_cty T)]) ->
   has_type Gamma (broadcast_exp e) T (seq (C :: [(s_cty T)])).
...
Inductive step : global_conf -> action -> global_conf -> Prop :=
 ...
 | ST_seq_exp2 : forall T x st' e1 M M' p P P' a e2 
   ...
   (Sigma st'' e2 M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v) |: P')
 | ST_broadcast_exp1 : forall v v' v'' v''' st' e' M' x T
   M p P,
   value v'' ->
   step (Sigma (st_decl T x v''') (broadcast_exp v'') 
   (v :: M) (val_exp p) |: 
   (Sigma st' e' M' v') |: P) 
   (broadcast v'' (val_exp p))
   (Sigma (st_decl T x v''') v'' (v :: M) (val_exp p) |: 
   (Sigma st' e' (append_mail_box M' v'') v') |: update_all_mail v'' P)
 | ST_broadcast_exp2 : forall v v' v''' st' e' M' x T
   M p P M'' e'' st'' M''' e''' st''' P' e a,
   step (Sigma (st_decl T x v''') e (v :: M) 
   (val_exp p) |: (Sigma st' e' M' v') |: P) a 
   (Sigma st'' e'' M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v') |: P') ->
   step (Sigma (st_decl T x v''') (broadcast_exp e) 
   (v :: M) (val_exp p) |: 
   (Sigma st' e' M' v') |: P) a 
   (Sigma st'' (broadcast_exp e'') M'' (val_exp p) |: 
   (Sigma st''' e''' M''' v') |: P').
...
Inductive appears_free_in : id -> exp -> Prop :=
 ...
 | afi_broadcast_exp : forall e x,
   appears_free_in x e ->
   appears_free_in x (broadcast_exp e).
...
apply T_val_exp.
- (* Broadcast *)
 apply T_broadcast_exp. 
 apply IHhas_type.
 intros. rewrite <- Heqv. 
 reflexivity.
 apply afi_broadcast_exp. 
 assumption. assumption.
Qed.
...
(* Broadcast *)
- right. destruct IHhas_type.
 apply fresh_address. inversion H1. assumption.
 reflexivity. exists (st_decl T' x' v'''). exists e.
 exists (v :: M). exists st'. 
 exists e'. exists (append_mail_box M' e).
 exists v'. exists (broadcast e (val_exp p)).
 exists (update_all_mail e P). exists T. 
 intros. 
 apply ST_broadcast_exp1.
 assumption. inversion H4. 
 exists x. inversion H5.
 exists (broadcast_exp x0). inversion H6. 
 exists x1. inversion H7.
 exists x2. inversion H8. 
 exists x3. inversion H9.
 exists x4. inversion H10. 
 exists x5. inversion H11.
 exists x6. inversion H12. 
 exists x7. inversion H13.
 exists x8. intros. 
 apply ST_broadcast_exp2.
...
Lemma mover_properties : forall a a' tr v p p' P P' tr1 tr2 K K1' K2' K'',
                   ...
                   ((a = (set p) -> lipton_reorder (append_actions tr1 (append_actions [a ; a'] tr2)) (append_actions tr1 (append_actions [a' ; a] tr2))))) \/
                   ((a' = (broadcast v p') -> lipton_reorder (append_actions tr1 (append_actions [a ; a'] tr2)) (append_actions tr1 (append_actions [a' ; a] tr2)))).
  Proof.
  intros.
  left.
  intros.
...
Lemma reduction_broadcast2 : forall T x v''' e v'' M p st' e' M' P a st'' 
e'' M'' st''' e''' M''' v' P',reduction (Sigma (st_decl T x v''') e (v'' :: M)(val_exp p) |: (Sigma st' e' M' v') |: P) a (Sigma st'' e'' M'' (val_exp p) |:(Sigma st''' e''' M''' v') |: P') ->reduction (Sigma (st_decl T x v''') (broadcast_exp e) (v'' :: M) (val_exp p) |:(Sigma st' e' M' v') |: P) a(Sigma st'' (broadcast_exp e'') M'' (val_exp p) |: (Sigma st''' e''' M''' v') |: P').
Proof.
 intros.
 inversion H. subst.
 apply one_step_red.
 ...
 subst.
 apply refl_red. subst.
 apply trans_red with st''0 (broadcast_exp e''0) M''0 P'0 st'''0 e'''0 M'''0.
 apply one_step_red. apply ST_broadcast_exp2.
 inversion H13. subst.
 assumption. inversion H21. subst.
 apply one_step_red. apply ST_broadcast_exp2.
 assumption.
...
Lemma reduction_broadcast1 : forall T x v''' v'' M p st' e' M' v' v P,value v'' ->reduction (Sigma (st_decl T x v''') (broadcast_exp v'') 
(v :: M) (val_exp p) |: 
(Sigma st' e' M' v') |: P) 
[(broadcast v'' (val_exp p))]
(Sigma (st_decl T x v''') v'' (v :: M) (val_exp p) |: 
